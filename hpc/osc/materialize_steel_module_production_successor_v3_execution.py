#!/usr/bin/env python3
"""Preview or materialize the incident-bound BC-S1 successor generation v3."""
...
def main() -> int:
    ...
    try:
        if args.check_only:
            preview = preview_successor_v3_execution(repo_root=repo_root)
            authority = preview.recovery_authority
            print("steel-module Phase-2B successor-v3 preview: PASS")
            print(f"predecessor_execution_id: {preview.predecessor.execution_id}")
            print(f"failed_r3_id: {preview.failure['failure_id']}")
            print(f"failed_r3_hash: {preview.failure['failure_hash']}")
            ...
            return 0
        execution = materialize_successor_v3_execution_companion(
            repo_root=repo_root
        )
    except (OSError, RuntimeError, ValueError, json.JSONDecodeError) as exc:
        print(f"Cannot materialize Phase-2B successor-v3: {exc}", file=sys.stderr)
        return 1
    print("managed steel-module Phase-2B successor-v3 materialization: PASS")
