#!/usr/bin/env python3
"""Preview or seal the fixed failed R3 preflight from external evidence files."""
...
def main() -> int:
    ...
    try:
        if args.check_only:
            payload, _ = collect_r3_failure_evidence(**kwargs)
            print("steel-module failed R3 preflight preview: PASS")
            print(f"failure_id: {payload['failure_id']}")
            print(f"failure_hash: {payload['failure_hash']}")
            print("job_id: 50544247")
            print("terminal: parent+batch FAILED / 1:0; extern COMPLETED / 0:0")
            ...
            return 0
        target = seal_r3_failure_evidence(**kwargs)
        payload = validate_r3_failure_bundle(
            target,
            execution_dir=args.execution_dir,
        ...
        print(f"Cannot seal failed steel-module R3 preflight: {exc}", file=sys.stderr)
        return 1
