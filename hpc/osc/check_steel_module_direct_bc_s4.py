#!/usr/bin/env python3
"""Focused, scheduler-free checks for the direct BC-S4 campaign route."""
...
import generate_steel_module_direct_bc_s4_campaign as generator
from check_steel_module_direct_bc_s1 import (
    fixture_environment,
    fixture_git_checkout,
)
from check_steel_module_direct_bc_s3 import (
    fixture_progression as bc_s3_fixture_progression,
    fixture_source as bc_s3_fixture_source,
    fixture_tasks as bc_s3_fixture_tasks,
)
...
from generate_steel_module_direct_bc_s4_campaign import (
    ...
    write_direct_bc_s4_campaign_atomic,
)
...
def fixture_analysis_records() -> tuple[
    ...
    primary = {
        "schema_version": (
            "steel-module-direct-bc-s3-cumulative-analysis-v1-primary-v1"
        ),
        "state_id": "BC-ONLY-S3",
        ...
        "interval_narrowing_vs_bc_s2": True,
        **EXPECTED_PRIMARY,
    }
    eligibility = {
        "schema_version": (
            "steel-module-direct-bc-s3-cumulative-analysis-v1-"
...
def fixture_progression() -> dict[str, object]:
    ...
    progression: dict[str, object] = {
        ...
        "predecessor_attempt_ids": ["fixture-bc-s3-attempt"],
...
def predecessor_binding_check(
    ...
    predecessor = write_direct_bc_s3_campaign_atomic(
        predecessor_dir,
        tasks=bc_s3_fixture_tasks(),
        environment=environment,
        source=bc_s3_fixture_source(bc_s3_fixture_tasks()),
        progression=bc_s3_fixture_progression(),
        git=git,
    ...
    try:
        loaded, progression = generator.load_bc_s3_progression(
        ...
        assert progression["next_child"] == "BC-S4"
        ...
        write_recursive_checksums(analysis)
        try:
            generator.load_bc_s3_progression(predecessor_dir, repo_root=checkout)
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="steel-module-direct-bc-s4-") as raw:
        scratch = Path(raw)
        ...
        assert "Tasks: 162 total, 0 submitted, 0 complete" in checked.stdout
        ...
        assert "Submitted Slurm array 888888 with 162 logical tasks." in submitted.stdout
        command = submitted_command.read_text(encoding="utf-8").splitlines()
        assert command[command.index("--array") + 1] == "1-162"
        ...
        bad_tasks[0] = replace(bad_tasks[0], seed_block=79)
        try:
            validate_direct_bc_s4_tasks(bad_tasks, geant4_version="11.4.2")
        except ValueError:
            pass
        else:
            raise AssertionError("wrong direct BC-S4 block range was accepted")

        primary, eligibility, decomposition = fixture_analysis_records()
        bad_primary = {**primary, "ratio": float(EXPECTED_PRIMARY["ratio"]) + 0.1}
        try:
            validate_progression_values(
                primary=bad_primary,
                eligibility=eligibility,
            ...
            raise AssertionError("changed BC-S3 primary result was accepted")
        ...
        try:
            ...
            raise AssertionError("existing direct BC-S4 campaign target was overwritten")

        predecessor_binding_check(scratch, checkout, environment, git)

    print(
        "steel-module direct BC-S4: PASS "
        "(162 tasks, 40500 events, reviewed continue, ordinary array, no preflight)"
    )
    return 0
