#!/usr/bin/env python3
"""Preview or materialize the rejected-probe-bound BC-S1 successor v4."""
...
def main() -> int:
    ...
    try:
        if args.check_only:
            preview = preview_successor_v4_execution(repo_root=repo_root)
            authority = preview.recovery_authority
            print("steel-module Phase-2B successor-v4 preview: PASS")
            print(f"predecessor_execution_id: {preview.predecessor.execution_id}")
            rejected = authority["rejected_r3_preflight"]
            print(f"rejected_r3_id: {rejected['rejection_id']}")
            print(f"rejected_r3_hash: {rejected['rejection_hash']}")
            ...
            return 0
        execution = materialize_successor_v4_execution_companion(
            repo_root=repo_root
        )
    except (OSError, RuntimeError, ValueError, json.JSONDecodeError) as exc:
        print(f"Cannot materialize Phase-2B successor-v4: {exc}", file=sys.stderr)
        return 1
    print("managed steel-module Phase-2B successor-v4 materialization: PASS")
    ...
    print("submission_ready: false (new R3/R4 required)")
    print("intents: 0")
