#!/usr/bin/env python3
"""Validate formal BC-S1 successor-v5 and its no-submission boundary."""
...
def main() -> int:
    ...
    try:
        ...
        if (
            ...
            raise ValueError("live and frozen successor-v5 admission disagree")
        validate_successor_r2_boundary(execution, repo_root=repo_root)
    except (OSError, RuntimeError, ValueError, json.JSONDecodeError) as exc:
        print(f"Cannot validate Phase-2B successor-v5: {exc}", file=sys.stderr)
        return 1
    authority = execution.manifest["recovery_authority"]
    print("steel-module Phase-2B successor-v5 validation: PASS")
    ...
    print(
        "preworkspace_failure_hash: "
        f"{authority['preworkspace_r3_failure']['failure_hash']}"
    )
    print("tasks: 32")
    print("events: 8000")
